...
interface StoryModalHomepageProps {
  open: boolean
...
export function StoryModalHomepage({
  open,
  setOpen,
  storyDataHome = [],
  userName,
  ...
  }, [currentIndex, isPlaying, isMuted, open, storyDataHome, videoDurations])
  ...
  const togglePlay = (index: number) => {
    ...
    const mediaType = detectMediaType(storyDataHome[index].fileName)
    if (mediaType === 'video') {
      const video = videoRefs.current[index]
      if (video) {
  ...
  const toggleMute = (index: number) => {
    ...
    const video = videoRefs.current[index]
    if (video) video.muted = newIsMuted[index]
  }
      ...
      <DialogContent className="max-w-full h-full sm:max-w-full sm:h-full p-0 border-none bg-transparent">
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
          ...
          <div className="relative w-full max-w-[400px] sm:max-w-[450px] md:max-w-[500px] h-[70vh] sm:h-[80vh] mx-auto">
            ...
            <div className="absolute top-4 left-0 right-0 flex items-center justify-between px-4 z-20">
              <div className="flex items-center gap-2">
                <Avatar className="w-7 h-7 sm:w-8 sm:h-8 border border-gray-500">
                  <AvatarImage
                    src={`https://instagram-api.softclub.tj/images/${userImage}`}
                    alt={userName}
                  />
                  <AvatarFallback>
            ...
            {/* Story content */}
            <div className="h-full w-full bg-black rounded-md overflow-hidden flex items-center justify-center">
              {currentMediaType === 'video' ? (
                <video
                  ref={el => {
