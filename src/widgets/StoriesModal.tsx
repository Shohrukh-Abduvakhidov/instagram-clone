...
import { IStoryData } from '@/features/Stories/model/types'
...
export function StoryModal({ storyData, open, setOpen }: StoryModalProps) {
	...
	const progressInterval = useRef<NodeJS.Timeout | number | undefined>(
		undefined
	)
	...
	useEffect(() => {
		if (storyData?.data?.stories) {
			...
			setVideoDurations(storyData.data.stories.map(() => 5))
		}
	...
	useEffect(() => {
		...
		const loadDurations = async () => {
			const durations = await Promise.all(
				storyData?.data.stories.map(
					async (story: IStoryData, index: number) => {
						...
						// For images, use a default duration (e.g., 5 seconds)
						return 5
					}
				)
			)
			...
			<DialogContent className='max-w-full h-full sm:max-w-full sm:h-full p-0 border-none bg-transparent'>
				<div className='fixed inset-0 bg-black/90 flex items-center justify-center z-50'>
					...
					<div className='absolute top-4 left-4 z-50'>
						<h1 className='text-[#fff] font-bold text-[25px]'>
							<svg
					...
					<div className='relative w-full max-w-[400px] sm:max-w-[450px] md:max-w-[500px] h-[70vh] sm:h-[80vh] mx-auto'>
						...
						<div className='h-full w-full bg-black rounded-md overflow-hidden'>
							{storyData?.data?.stories.length > 0 &&
								(isVideoFile(
									storyData?.data?.stories[currentIndex].fileName
								) ? (
									<video
										ref={el => {
