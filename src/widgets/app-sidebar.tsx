...
export default function AppSidebar() {
	...
	if (isMobile) {
		return (
			<>
				{/* Mobile top bar */}
				<div className='fixed top-0 left-0 right-0 bg-black border-b flex justify-between items-center h-12 px-4 z-20'>
				...
				<nav className='fixed bottom-0 left-0 right-0 bg-black border-t flex justify-around items-center h-16 px-2 z-10'>
					...
					<Link
						...
						className='flex flex-col items-center justify-center p-2'
					>
						<Search className='w-6 h-6' /> {/* Иконка Search для explore */}
					</Link>
					<button
						className='flex flex-col items-center justify-center p-2'
	...
	return (
		...
		>
			...
			<button
				...
				className='flex items-center p-3 rounded-md hover:bg-[#b3adad4b] transition-colors mt-auto mb-2 mx-2'
			>
				<PanelLeftIcon size={20} />
				{expanded && <span className='pl-5'>Свернуть</span>}
			</button>
		</nav>
	)
}
